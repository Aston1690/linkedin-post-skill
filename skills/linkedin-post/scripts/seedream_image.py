#!/usr/bin/env python3
"""
Seedream 4.5 Image Generation Script for LinkedIn Post Skill.

Uses ByteDance's Seedream 4.5 model via OpenRouter's chat/completions endpoint
to generate high-quality images from detailed text prompts.

Usage:
    python seedream_image.py --prompt "Your design prompt" --output "./output/post.png"
    python seedream_image.py --prompt "..." --output "..." --aspect portrait
"""
...
def get_api_key():
    """Get the Seedream API key from environment."""
        ...
        for env_path in [".env", "../.env", "../../.env",
            ...
            if os.path.exists(env_path):
                with open(env_path) as f:
                    for line in f:
                        line = line.strip()
                        if line.startswith("SEADREAM_API_KEY="):
                            key = line.split("=", 1)[1].strip().strip('"').strip("'")
                            break
                if key:
                    break
    if not key:
        print("Error: SEADREAM_API_KEY not found. Set it as an environment variable or in .env file.",
              file=sys.stderr)
        sys.exit(1)
    return key
...
def generate_image(prompt, width, height, api_key):
    """Generate image via OpenRouter chat completions with Seedream 4.5."""
...
def main():
    parser = argparse.ArgumentParser(description="Generate LinkedIn images with Seedream 4.5 via OpenRouter")
    ...
    parser.add_argument("--reference", help="(ignored — Seedream does not support reference images)")
    ...
    print(f"Generating with Seedream 4.5 via OpenRouter...")
